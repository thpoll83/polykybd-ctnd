...
- [ ] Discover actual QMK VID/PID for PolyKybd and update `station/config.py` (check `keyboards/polykbd/config.h` in `qmk_firmware`)
- [ ] Verify `USB_HUB_LOCATION`, `LEFT_USB_PORT`, `RIGHT_USB_PORT` by running `uhubctl` on the target RPi4
- [ ] Write concrete test cases in `test_runner.py` — currently the test list is empty; tests receive a `RawHID` instance and a `log` callback, return `True`/`False`
- [ ] Add GPIO-driven key matrix simulation (MCP23017 I2C expander or direct RPi GPIO wired to exposed switch pads) so tests can simulate actual key presses
- [ ] Add the CI workflow file (`.github/workflows/qmk-test.yml` from this repo) to `thpoll83/qmk_firmware`
- [ ] Confirm the QMK `config.h` has `#define MASTER_LEFT` before first HIL run
